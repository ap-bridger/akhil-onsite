    ...
    <tr className={`border-b border-gray-100 transition-colors ${isDirty ? "bg-amber-50 hover:bg-amber-100/60" : "hover:bg-gray-50"}`}>
      ...
      <td className="px-4 py-3 w-44">
        <div className="flex gap-2">
          <button onClick={() => onApprove(transaction.id)} disabled={!isDirty && status === "REVIEWED"}
          ...
          <button onClick={() => onUpdate(transaction.id, { status: "NEEDS_MORE_INFO" })} disabled={status === "NEEDS_MORE_INFO" && !isDirty}
